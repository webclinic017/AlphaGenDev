...
class St(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.information_sets = load_data(re_load = self.params.reload, experimental = self.params.experimental)
        self.last_tickets    = []
    ...
    def next(self):
        ...
        # Check if the signal files exist, otherwise we cannot trade
        yyyy = t.year
        mm   = t.month
        dd   = t.day
        ...
        if  (t.weekday() == 3) and (self.trades_in_month == 0):
            if self.params.experimental:
                signal_file=f"{PATH_TO_SEC_DATA}\\signals\\v1\\signalVS{yyyy}-{mm}-{dd}.dta"
            else:
                signal_file=f"{PATH_TO_SEC_DATA}\\signals\\v1\\signal{yyyy}-{mm}-{dd}.dta"
            ...
            if file_exists:
                try:
                    ...
                    tickers_long, tickers_short, tickers= retrieve_portfolio(t, self.information_sets,
                                                                            ...
                                                                            experimental = self.params.experimental)
